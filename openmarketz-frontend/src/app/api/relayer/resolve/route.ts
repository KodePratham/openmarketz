...
export async function POST(request: Request) {
  try {
    ...
    const tx = await market.resolve(body.req, body.signature);
    const receipt = await tx.wait();

    return NextResponse.json({ ok: true, txHash: receipt.hash });
  } catch (error) {
    console.error("Relayer resolve failed", error);
    return NextResponse.json(
      { ok: false, error: (error as Error)?.message ?? "Could not resolve market." },
      { status: 400 },
    );
  }
}
