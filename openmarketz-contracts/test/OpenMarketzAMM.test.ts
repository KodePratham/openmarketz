...
describe("OpenMarketzAMM", function () {
  ...
  async function deployFixture() {
    ...
    const mintAmount = u("1000000");
    await Promise.all([
      token.mint(creator.address, mintAmount),
      token.mint(traderA.address, mintAmount),
      token.mint(traderB.address, mintAmount),
      token.mint(randomUser.address, mintAmount),
      token.connect(creator).approve(await treasury.getAddress(), mintAmount),
    ]);

    const factory = await ethers.getContractFactory("OpenMarketzAMM", treasury);
    const contract = await factory.deploy(treasury.address, await token.getAddress());
    await contract.waitForDeployment();

    const contractAddress = await contract.getAddress();
    await Promise.all([
      token.connect(creator).approve(contractAddress, mintAmount),
      token.connect(traderA).approve(contractAddress, mintAmount),
      token.connect(traderB).approve(contractAddress, mintAmount),
      token.connect(randomUser).approve(contractAddress, mintAmount),
      token.connect(treasury).approve(contractAddress, mintAmount),
    ]);

    return { contract: contract as any, token: token as any, treasury, creator, traderA, traderB, randomUser };
  }

  it("creates market with 2 USDC seed and creator-only liquidity add", async function () {
    const { contract, creator, traderA } = await deployFixture();
    const now = (await ethers.provider.getBlock("latest"))!.timestamp;

    const tx = await contract
      .connect(creator)
      .createMarket("Will there be rain?", "Weather market", now + 3600, u("2"));
    ...
    await expectRevert(contract.connect(traderA).addLiquidity(1, u("1")));

    await contract.connect(creator).addLiquidity(1, u("1"));

    const market = await contract.getMarket(1);
    expect(market.collateralPool).to.equal(u("3"));
  ...
  it("tracks participated markets without duplicates across repeated buys", async function () {
    ...
    await contract.connect(creator).createMarket("Will volume rise?", "portfolio indexing", now + 3600, u("2"));
    ...
    await contract.connect(traderA).buyYes(1, shares, yesGross + yesFee);
    ...
    await contract.connect(traderA).buyNo(1, shares, noGross + noFee);
  ...
  it("applies 0.5% trade fee and splits 70/30 LP and treasury", async function () {
    const { contract, token, creator, traderA, treasury } = await deployFixture();
    const now = (await ethers.provider.getBlock("latest"))!.timestamp;

    await contract.connect(creator).createMarket("Will token pump?", "Fee split check", now + 3600, u("2"));
    ...
    await contract.connect(traderA).buyYes(1, shares, gross + fee);
    ...
    await contract.connect(treasury).claimTreasuryTradeFees(1);
    const treasuryAfter = await token.balanceOf(treasury.address);

    expect(treasuryAfter - treasuryBefore).to.equal(treasuryClaimable);
  });

  it("charges strict 2% winner fee on profit only", async function () {
    const { contract, token, creator, traderA, traderB, treasury } = await deployFixture();
    const now = (await ethers.provider.getBlock("latest"))!.timestamp;

    await contract.connect(creator).createMarket("Will team A win?", "Winner fee check", now + 20, u("2"));
    ...
    const yesFee = (yesGross * 50n) / 10_000n;
    await contract.connect(traderA).buyYes(1, yesShares, yesGross + yesFee);

    const noPrice = await contract.getImpliedPriceBps(1, false);
    const noGross = (noShares * noPrice) / 10_000n;
    const noFee = (noGross * 50n) / 10_000n;
    await contract.connect(traderB).buyNo(1, noShares, noGross + noFee);
    ...
    const treasuryBefore = await token.balanceOf(treasury.address);
    await contract.connect(treasury).claimTreasuryWinnerFees(1);
    const treasuryAfter = await token.balanceOf(treasury.address);

    expect(treasuryAfter - treasuryBefore).to.equal(winnerFeesAfter);
  });

  it("auto-cancels after grace period and allows net cash refunds", async function () {
    const { contract, token, creator, traderA } = await deployFixture();
    const now = (await ethers.provider.getBlock("latest"))!.timestamp;

    await contract.connect(creator).createMarket("Will this get unresolved?", "cancel path", now + 60, u("2"));
    ...
    const before = await token.balanceOf(traderA.address);
    await contract.connect(traderA).claimCancelRefund(1);
    const after = await token.balanceOf(traderA.address);

    expect(after - before).to.equal(gross + fee);
  });
});